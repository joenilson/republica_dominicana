...
class informe_facturas extends fs_controller
{
   ...
   private function informe_ventas()
   {
      ...
      if($data)
      {
         ...
         header("Content-Disposition: attachment; filename=\"informe_ventas".".xlsx\"");
         header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
         header('Content-Transfer-Encoding: binary');
         header('Cache-Control: must-revalidate');
         header('Pragma: public');
         ...
         */
         ...
         $writer = new XLSXWriter();
         $writer->setAuthor('Generador Excel FS');
         $writer->writeSheetHeader('Informe Ventas', $header);
         ...
         foreach($stats as $i => $value)
         {
            ...
            foreach($value as $j => $value2)
            {
               if($cli)
               {
                  ...
                  $linea[] = $i;
                  $linea[] = $this->fix_html($cli->nombre);
                  $linea[] = $j;
               }
               else
               {
               ...
               foreach($value2 as $value3)
               {
                  //echo ';'.show_numero($value3, FS_NF0);
                  $linea[] = $value3;
               }
               //echo "\n";
            }
            //echo ";;;;;;;;;;;;;;;\n";
            $writer->writeSheetRow('Informe Ventas', $linea);
         }
         $total_xls = array();
         foreach( array_reverse($totales, TRUE) as $i => $value)
         {
         ...
         $lineas++;
         $writer->writeSheetRow('Informe Ventas', $total_xls, $estilo_pie);
         $writer->markMergedCell('Informe Ventas', 0, $lineas, 0, $lineas+1);
         $writer->writeToStdOut();
      }
      else
      {
   ...
   private function informe_compras_unidades()
   {
      ...
      if($data)
      {
         ...
         header("Content-Disposition: attachment; filename=\"informe_compras_unidades".".xlsx\"");
         header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
         header('Content-Transfer-Encoding: binary');
         header('Cache-Control: must-revalidate');
         header('Pragma: public');
         ...
         */
         ...
         $writer = new XLSXWriter();
         $writer->setAuthor('Generador Excel FS');
         $writer->writeSheetHeader('Informe Compras', $header);
            ...
            foreach($value as $j => $value2)
            {
               ...
               foreach($value2 as $k => $value3)
               {
                  if($pro)
                  {
                     ...
                     $linea[] = $i;
                     $linea[] = $this->fix_html($pro->nombre);
                     $linea[] = $j;
                     $linea[] = $k;
                  }
                  else
               ...
               $writer->writeSheetRow('Informe Compras', $linea);
               //echo ";;;;;;;;;;;;;;;\n";
            }
            //echo ";;;;;;;;;;;;;;;\n";
