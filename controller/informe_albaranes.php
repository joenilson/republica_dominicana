...
class informe_albaranes extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        if (isset($_REQUEST['buscar_cliente'])) {
            ...
            if (isset($_POST['generar'])) {
                ...
                } elseif ($_POST['generar'] === 'xls_cli') {
                    $this->generar_xls('venta');
                } elseif ($_POST['generar'] === 'csv_cli') {
                    $this->generar_csv('venta');
                } elseif ($_POST['generar'] === 'pdf_prov') {
                    $this->generar_pdf('compra');
                } elseif ($_POST['generar'] === 'xls_prov') {
                    $this->generar_xls('compra');
                } elseif ($_POST['generar'] === 'csv_prov') {
    ...
    public function stats_last_30_days()
    {
        ...
        foreach ($stats_cli as $i => $value) {
            $mes = $meses[(int)date('m', strtotime($value['day']))];
            $stats[$i] = array(
    ...
    public function stats_months()
    {
        ...
        foreach ($stats_cli as $i => $value) {
            $stats[$i] = array(
                'month' => $meses[(int)$value['month']] . $value['year'], /// el identificador del mes es mes+año
                'total_cli' => round($value['total'], FS_NF0),
                'total_pro' => 0
            );
        }
    ...
    protected function stats_months_aux($table_name)
    {
        ...
        $sql = "SELECT " . $sql_aux . " as mes, SUM(neto) as total FROM " . $table_name
            . $where . " AND anulada = false GROUP BY " . $sql_aux . " ORDER BY mes ASC;";
    ...
    protected function stats_years_aux($table_name, $num = 4)
    {
        ...
        $data = $this->db->select("SELECT " . $sql_aux . " as ano, sum(neto) as total FROM " . $table_name
            . $where . " AND anulada = false GROUP BY " . $sql_aux . " ORDER BY ano ASC;");
    ...
    public function stats_series($tabla)
    {
        ...
        $sql .= $where;
        $sql .= " AND anulada = false group by codserie order by total desc;";
            ...
            foreach ($data as $d) {
                ...
                if ($serie) {
                    $stats[] = array(
                        ...
                        'total' => round((float)$d['total'], FS_NF0)
                    );
                } else {
                    $stats[] = array(
                        'txt' => $d['codserie'],
    ...
    public function stats_agentes($tabla)
    {
        ...
        $sql .= $where;
        $sql .= " AND anulada = false group by codagente order by total desc;";
    ...
    public function stats_almacenes($tabla)
    {
        ...
        $sql .= $where;
        $sql .= " AND anulada = false group by codalmacen order by total desc;";
    ...
    public function stats_formas_pago($tabla)
    {
        ...
        $sql .= $where;
        $sql .= " AND anulada = false group by codpago order by total desc;";
    ...
    protected function get_documentos($tabla)
    {
        ...
        $sql = "select * from " . $tabla . $where . " and anulada = false order by fecha asc, codigo asc;";
        $data = $this->db->select($sql);
        if ($data) {
            foreach ($data as $d) {
                if ($tabla === $this->table_ventas) {
